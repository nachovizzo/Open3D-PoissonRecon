...
class CholmodSolver {
    ...
    template <class Real, class MatrixRowIterator>
    void _init(const SparseMatrixInterface<Real, MatrixRowIterator>& M);
...
public:
    ...
    void solve(ConstPointer(Real) b, Pointer(Real) x);
    template <class Real, class MatrixRowIterator>
    bool update(const SparseMatrixInterface<Real, MatrixRowIterator>& M);
...
CholmodSolver::CholmodSolver(
        ...
        bool analyzeOnly) {
    _init(M);
    if (!analyzeOnly) update(M);
}
template <class Real, class MatrixRowIterator>
void CholmodSolver::_init(
        const SparseMatrixInterface<Real, MatrixRowIterator>& M) {
    {
    ...
    if (LOWER_TRIANGULAR) {
        ...
        cholmod_M = CHOLMOD(allocate_sparse)(dim, dim, maxEntries, 0, 1, -1,
                                             CHOLMOD_REAL, &cholmod_C);
    } else {
        maxEntries = (int)M.entries();
        cholmod_M = CHOLMOD(allocate_sparse)(dim, dim, maxEntries, 0, 1, 0,
                                             CHOLMOD_REAL, &cholmod_C);
    }
    ...
    for (int i = 0; i < M.rows(); i++) {
        _p[dim++] = off;
        for (MatrixRowIterator iter = M.begin(i); iter != M.end(i); iter++)
            if (!LOWER_TRIANGULAR || iter->N >= i) _i[off++] = iter->N;
    }
    _p[dim] = off;

    cholmod_L = CHOLMOD(analyze)(cholmod_M, &cholmod_C);
    cholmod_b =
            CHOLMOD(allocate_dense)(dim, 1, dim, cholmod_M->xtype, &cholmod_C);
}
template <class Real, class MatrixRowIterator>
bool CholmodSolver::update(
        const SparseMatrixInterface<Real, MatrixRowIterator>& M) {
...
#pragma omp parallel for
    for (int i = 0; i < M.rows(); i++) {
        int off = (int)_p[i];
        for (MatrixRowIterator iter = M.begin(i); iter != M.end(i); iter++)
            if (!LOWER_TRIANGULAR || iter->N >= i)
                _x[off++] = double(iter->Value);
    }
    ...
    if (cholmod_C.status == CHOLMOD_NOT_POSDEF) {
        fprintf(stderr,
                "[WARNING] CholmodSolver::update: Matrix not "
                "positive-definite\n");
        return false;
    } else if (cholmod_C.status == CHOLMOD_OUT_OF_MEMORY) {
        fprintf(stderr,
                "[WARNING] CholmodSolver::update: CHOLMOD ran out of memory\n");
        return false;
    } else if (cholmod_C.status != CHOLMOD_OK) {
        fprintf(stderr,
                "[WARNING] CholmodSolver::update: CHOLMOD status not OK: %d\n",
                cholmod_C.status);
        return false;
    }
    return true;
...
CholmodSolver::~CholmodSolver(void) {
    if (cholmod_L)
        CHOLMOD(free_factor)(&cholmod_L, &cholmod_C), cholmod_L = NULL;
    if (cholmod_b)
        CHOLMOD(free_dense)(&cholmod_b, &cholmod_C), cholmod_b = NULL;
    if (cholmod_M)
        CHOLMOD(free_sparse)(&cholmod_M, &cholmod_C), cholmod_M = NULL;
}
...
void CholmodSolver::solve(ConstPointer(Real) b, Pointer(Real) x) {
    ...
    cholmod_dense* cholmod_x =
            CHOLMOD(solve)(CHOLMOD_A, cholmod_L, cholmod_b, &cholmod_C);
...
int CholmodSolver::nonZeros(void) const {
    long long nz = 0;
    if (cholmod_L->xtype != CHOLMOD_PATTERN && !(cholmod_L->is_super))
        for (int i = 0; i < cholmod_L->n; i++)
            nz += ((SOLVER_LONG*)cholmod_L->nz)[i];
    bool examine_super = false;
    if (cholmod_L->xtype != CHOLMOD_PATTERN)
        examine_super = true;
    else
        examine_super = (((int*)cholmod_L->s)[0] != (-1));
    if (examine_super) {
